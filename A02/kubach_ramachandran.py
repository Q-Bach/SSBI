...
args_parser.add_argument(
    "-o", "--output",
    dest="output",
    help="Paths to output. Ending specifies file type (e.g. .pdf).",
    type=str,
    required=True
)
...
def main():
    ...
    # List for phi_psi pairs:
    phi_psi = calc_phi_psi(pdbs)

    # save plot
    plot_ramachandran(phi_psi, args.output, os.path.splitext(os.path.basename(args.output))[0])
...
def calc_phi_psi(pdbs: []) -> list:
    """
    Iterates over a list of pdb objects, their models, chains and residues.
    For each enclosed residue, the phi and psi angle is calculated and stored
    in a two dimensional array.
    :param pdbs: list with pdb objects
    :return: [[phi, psi], [phi, psi], ...]
    """
    phi_psi = []
    # iterating over all PDB files:
    for pdb in pdbs:
...
def plot_ramachandran(phi_psi: list, path: str, title= "Ramachandran Plot") -> None:
    """
    Generates a ramachandran plot with mathplotlib and saves it to a defined file.
    :param title: Title of the diagram
    :param phi_psi: two dimensional list with phi and psi
